...
def Evaluation(output, labels):
    ...
    # '''
    return metrics.f1_score(labels, binary_pred, average="micro"), metrics.f1_score(labels, binary_pred,
                                                                                    average="macro")
...
def save_model(config, dataset_name, model):
    PATH_MODEL = os.path.join(os.path.join(os.getcwd(), 'model_saved'), dataset_name,
...
def save_best_val_model_all(config, dataset_name, model, PATH_MODEL, current_metric, best_metric, best_val_paths,
                            b_epoch):
    """
...
def load_model_all(dataset, model, loss_func, model_paths, T):
    ...
    for i, path in enumerate(model_paths):
        ...
        res = T.evaluation(dataset.testing_graphs, dataset.trainval_graphs, model, dataset)
        met_test[i] = res[i]
    return met_test
...
if __name__ == "__main__":
    ...
    mydir = os.path.join(os.path.join(os.getcwd(), 'model_saved'),
                         datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    try:
        os.makedirs(mydir)
    except OSError as e:
